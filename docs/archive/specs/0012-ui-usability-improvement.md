...
상태: 사용자 승인 완료, Phase 9 구현 계획 작성 완료, 구현 전
...
- macOS와 Windows의 데스크톱 창 크기 변화에 맞춰 작업 영역이 깨지지 않게 조정된다.
...
- 외부 여백은 표준 창 24px, compact 창 16px
- 섹션 간격 16px
...
- compact 창에서도 목록과 상세를 유지하되 목록 너비를 줄이고 상세 영역을 우선한다.
...
Chrona의 대상은 macOS와 Windows 데스크톱 앱이다. 모바일·태블릿 전용 레이아웃은 구현하지 않는다. 대신 창 크기 조절, 작은 노트북 화면, Windows 디스플레이 배율에서 작업 영역이 깨지지 않도록 한다.

Tauri 창 최소 크기는 `960×640`으로 정한다. 이보다 작은 모바일형 화면은 지원 범위가 아니다.

### 넓은 창: 1440px 이상

- 사이드바 232~240px
- 본문 최대 너비 제한 없이 작업 영역 사용
- 파일과 스냅샷 master-detail을 넉넉한 비율로 표시
- 표와 상세의 독립 스크롤 유지

### 표준 창: 1100~1439px

- 사이드바 208~220px
- 상단 저장소 이름과 경로는 사용 가능한 너비에 맞춰 말줄임
- 파일 목록과 상세를 약 42:58 비율로 유지
- 본문 외부 여백 24px

### Compact 창: 960~1099px

- 사이드바는 아이콘과 짧은 명칭을 유지하되 176~192px로 축소
- 일반 설명 문구와 비핵심 보조 정보는 한 줄 말줄임
- 파일 목록과 상세를 약 36:64 비율로 유지
- 본문 외부 여백 16px
- 상단 작업 버튼은 아이콘과 짧은 명칭을 사용하되 숨기지 않음

### 세로 크기 대응

- 상단바와 사이드바는 고정
- 본문, 파일 목록, 파일 상세는 각자 정해진 영역에서 스크롤
- 하단 작업 표시는 실행 중에만 본문 높이에서 자신의 높이를 차지함
- 최소 높이 640px에서 제목, 주요 동작, 첫 작업 행이 동시에 보이도록 함
...
- macOS: 960×640, 1100×800, 1440×900 라이트/다크
- Windows: 960×640, 1100×800, 1440×900 라이트/다크
- Windows 125% 디스플레이 배율에서 텍스트와 버튼 잘림 확인
- 최소 창에서 본문 제목, 주요 동작, 첫 작업 행이 보이는지 확인
...
- 960×640 최소 창에서 탐색과 본문 작업이 겹치지 않는다.
...
1. 저장소 등록 데이터와 기본 경로
2. 앱 셸, 상단바, 사이드바, 데스크톱 창 크기별 스크롤 구조
3. 저장소 시작·전환·생성·불러오기
4. 홈과 새 백업 흐름
